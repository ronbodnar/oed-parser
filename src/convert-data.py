...
def populate_words():
>>>>>>> be8b0d6 (Changed words data structure in convert-data.py to a dictionary to efficiently ignore duplicate words. Added barebones functions for writing output to csv and raw words to a txt file.)
...
write_words()
>>>>>>> 9b03a08 (Added parsed-data outputs in CSV, JSON, and txt format. Also added a words.txt with a newline separated list of all words (without duplicates).)
=======
import csv
import json
import re
