    ...
    try:
        ...
        if response.status_code != 200:
            ...
            if not retry and num_retries <= max_retries:
                ...
                num_retries += 1
                print(f" | Retrying after {error_delay} seconds... ({num_retries})")
...
def get_parsed_content(content):
    ...
    for item in results_set_items:
        # Create a blank Word object
        word = Word()
        
        # Extract the word and set it as the word text if found.
        word_span = item.find('span', class_='hw')
        if word_span:
            word.text = word_span.get_text().strip()
        
        # Extract the snippet and set it as the word snippet if found.
        snippet_div = item.find('div', class_='snippet')
        if snippet_div:
            word.snippet = snippet_div.get_text().strip().replace('"', "'")
        
        # Extract the part of speech and set it as the word part if found.
...
def start_parsing(starting_page, max_pages, max_retries, request_delay, error_delay, output_file, delimiter):
    ...
    content is available. The function pauses for response_delay second(s) between requests to avoid
    overwhelming the server. Additionally, this function creates the output_file directory if necessary.
...
def save_parsed_content(output_file, parsed_content, current_page, delimiter):
    ...
    for i in range(len(parsed_content)):
        # Set the text, snippet, and parts_of_speech from the word at the index in parsed_content.
        text = parsed_content[i].text
        snippet = parsed_content[i].snippet
        parts_of_speech = parsed_content[i].parts_of_speech
        
        # Append the current page and word data, separated by a delimiter.
        lines.append(f'{current_page}{delimiter}"{text}"{delimiter}"{snippet}"{delimiter}"{parts_of_speech}"\n')
    
    # Output the list of lines to the output file.
    with open(output_file, 'a', encoding='utf-8') as f:
        f.writelines(lines)

# Only run the main parser if the script is ran directly.
